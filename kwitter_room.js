...
function getData() { firebase.database().ref("/").on('value',
 ...
 snapshot.forEach(function(childSnapshot) { childKey = childSnapshot.key; 
      Room_names = childKey;// Room_names will be having the name of the room which is coming from the firebase. console.log("Room Name - " + Room_names); row = "<div class='room_name' id="+Room_names+" onclick='redirectToRoomName(this.id)' >#"+ Room_names +"</div><hr>"; document.getElementById("output").innerHTML += row; //to display room name on kwitter room.html }); }); } getData();
